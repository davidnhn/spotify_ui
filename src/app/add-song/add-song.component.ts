...
import {CreateSongFormContent} from "../service/model/add-song-form.model";
  ...
  constructor() {
    effect(() => {
      ...
      if(this.songService.addSig().status === "OK") {
        ...
        this.toastService.showToast('Song created with success', 'SUCCESS');
        this.router.navigate(['/']);
      } else if(this.songService.addSig().status === "ERROR") {
        this.toastService.showToast('Error occured when creating song, please try again', 'DANGER');
      }
    });
  }
